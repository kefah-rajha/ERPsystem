...
interface showCategories {
    subCategoryData: categoriesResopnseData;
    handleDeleteSubCategory:(id:string)=>void,
    handleUpdateSubCategory:(data:categoriesResopnseData)=>void
}

function ShowSubCategories({ subCategoryData ,handleUpdateSubCategory,handleDeleteSubCategory}: showCategories) {
  ...
  const sendDataToServer = async () => {
    ...
    const fetchData = await fetch(`/api/Subcategory/UpdateSubCategory/${subCategoryData._id}`, {
      method: "POST",
      headers: {
    ...
    if(res.success == true){
        setOpen(false)
    handleUpdateSubCategory(res.data)

}

  };}
  const deleteCategory = async(id :string)=>{
    const fetchDeleteData=await fetch(  `/api/subCategory/deleteSubCategory/${subCategoryData.parent}/${id}`,{
        method:"Delete",
    ...
    >
        ...
        <div className=" absolute card-gradient  text-red-400 hover:text-foreground hover:bg-red-800    shadow-lg    transition-opacity -right-6 -top-1 z-30 rounded-full  h-8 w-8 flex justify-center items-center">
          X
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>
                    Are you absolutely sure ,Delete {subCategoryData.name}?
                  </AlertDialogTitle>
                  <AlertDialogDescription>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction>
                     <Button onClick={() => deleteCategory(subCategoryData._id)}>
                              Continue
                            </Button> 
                  </AlertDialogAction>
                </AlertDialogFooter>
      ...
      <div className="h-14 flex items-center justify-between hover:bg-orange-50 bg-gradient w-full rounded-sm shadow-lg pl-2 mt-4 ">
        ...
        <div
          ...
          onClick={() => {
            push(`/dashboard/Products/Categories/${subCategoryData._id}`);
          }}
        >
