...
function CreateSupplierForm() {
  ...
  async function onSubmit(dataForm: profileFormValues) {
    ...
    const fetchdata = await fetch("/api/supplier/createSupplier", {
      method: "POST",
      headers: {
    ...
    const createDataProfileInfo = await fetchdata.json();

    console.log(createDataProfileInfo, "updateDataProfileInfo");
    if (createDataProfileInfo.success == true) {
      ...
      supplierContext?.setSupplier([...supplierContext.supplier, createDataProfileInfo.data]);
    } else {
      toast({
        variant: "custum",
